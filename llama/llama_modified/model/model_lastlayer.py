...
def BlockQuantize2D(data, g, calculate_bias, m = 8, e = 8, bias = 0):
  tensor = data.clone()
  [a, b] = tensor.shape
  [c, d] = g
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        wv_shape = ColumnParallelLinear(
            in_features=self.wv.in_features,
            out_features=self.wv.out_features,
            bias=False,
            gather_output=False,
            init_method=lambda x: x,
        )
        ...
        group_shape = [inputs[2], inputs[3]]
        print(inputs)
        quantized_weights = BlockQuantize2D(self.wq.get_master_weight(), group_shape, 1, inputs[0], inputs[1])
        wq_shape.weight.copy_(quantized_weights)
        quantized_weights = BlockQuantize2D(self.wk.get_master_weight(), group_shape, 1, inputs[0], inputs[1])
        wk_shape.weight.copy_(quantized_weights)
        quantized_weights = BlockQuantize2D(self.wv.get_master_weight(), group_shape, 1, inputs[0], inputs[1])
        wv_shape.weight.copy_(quantized_weights)
        
        xq, xk, xv = wq_shape(x), wk_shape(x), wv_shape(x)
        ...
        quantized_weights = BlockQuantize2D(self.wo.get_master_weight(), group_shape, 1, inputs[0], inputs[1])
        wo_shape.weight.copy_(quantized_weights)

        output = wo_shape(output)
        
        if self.layer_id==31:
            print('ATTENTION_OUTPUT_ACTIVATION: ',output.shape)
            bfp_size = inputs[0] + inputs[1]
            group_size = group_shape[0]*group_shape[1]
            group_shape_str = str(group_shape[0])+'x'+str(group_shape[1])
            layer_filename = 'quantize/bfp'+str(bfp_size)+'/size_'+str(group_size)+'/shape_'+group_shape_str+'/layer_'+str(self.layer_id)+'_attention_output.pt'
...
class FeedForward(nn.Module):
    ...
    def forward(self, x, inputs):        
        ...
        w3_shape = RowParallelLinear(
            self.w3.in_features, self.w3.out_features, bias=False, input_is_parallel=True, init_method=lambda x: x
        )
        group_shape = [inputs[2], inputs[3]]
        quantized_weights = BlockQuantize2D(self.w1.get_master_weight(), group_shape, 1, inputs[0], inputs[1])
        w1_shape.weight.copy_(quantized_weights)  
        quantized_weights = BlockQuantize2D(self.w3.get_master_weight(), group_shape, 1, inputs[0], inputs[1])
        w3_shape.weight.copy_(quantized_weights)    
        quantized_weights = BlockQuantize2D(self.w2.get_master_weight(), group_shape, 1, inputs[0], inputs[1])
        w2_shape.weight.copy_(quantized_weights)
           
        w1out = w1_shape(x)
        w3out = w3_shape(x)
        tempout = F.silu(w1out) * w3out
        output = w2_shape(tempout)
        if self.layer_id==31:
            print('FFN_OUTPUT: ',output.shape)
            bfp_size = inputs[0] + inputs[1]
            group_size = group_shape[0]*group_shape[1]
            group_shape_str = str(group_shape[0])+'x'+str(group_shape[1])
            layer_filename = 'quantize/bfp'+str(bfp_size)+'/size_'+str(group_size)+'/shape_'+group_shape_str+'/layer_'+str(self.layer_id)+'_ffn_output.pt'
            append_and_save(output.squeeze(),layer_filename)
...
class TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        """
        h = x + self.attention(
            self.attention_norm(x), start_pos, freqs_cis, mask, inputs
        )
        out = h + self.feed_forward(self.ffn_norm(h), inputs)
        return out
...
class Transformer(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor, start_pos: int, inputs: List[int]):
        """
        ...
        for layer in self.layers:
            ...
            h = layer(h, start_pos, freqs_cis, mask, inputs)
