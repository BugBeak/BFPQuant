...
class Attention(nn.Module):
    ...
    def forward(
        ...
        # layer_filename = 'activations/layer_'+str(self.layer_id)+'_attention_input.pt'
        ...
        layer_filename = 'activations/new/layer_'+str(self.layer_id)+'_attention_output.pt'
        append_and_save(output.flatten(),layer_filename)
...
class FeedForward(nn.Module):
    ...
    def forward(self, x):
        ...
        # layer_filename = 'activations/layer_'+str(self.layer_id)+'_ffn_input.pt'
        # append_and_save(x.squeeze(),layer_filename)
        
        w1out = self.w1(x)
        w3out = self.w3(x)
        siluoutput = F.silu(w1out) 
        tempout = siluoutput * w3out
        output = self.w2(tempout)

        print('FFN_OUTPUT: ',output.shape)
        layer_filename = 'activations/new/layer_'+str(self.layer_id)+'_ffn_output.pt'
        append_and_save(output.flatten(),layer_filename)
