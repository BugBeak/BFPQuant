...
class Attention(nn.Module):
    ...
    def forward(
        ...
        quantized_weights = BlockQuantize2D(self.wq.get_master_weight(), group_shape, 1)
        wq_shape.weight.copy_(quantized_weights)
        quantized_weights = BlockQuantize2D(self.wk.get_master_weight(), group_shape, 1)
        wk_shape.weight.copy_(quantized_weights)
        quantized_weights = BlockQuantize2D(self.wv.get_master_weight(), group_shape, 1)
        ...
        quantized_weights = BlockQuantize2D(self.wo.get_master_weight(), group_shape, 1)
        wo_shape.weight.copy_(quantized_weights)

        output = wo_shape(output)
        
        print('ATTENTION_OUTPUT_ACTIVATION: ',output.shape)
        layer_filename = 'quantize/new/layer_'+str(self.layer_id)+'_attention_output.pt'
...
class FeedForward(nn.Module):
    ...
    def forward(self, x):        
        ...
        quantized_weights = BlockQuantize2D(self.w1.get_master_weight(), group_shape, 1)
        ...
        layer_filename = 'quantize/new/layer_'+str(self.layer_id)+'_ffn_output.pt'
        append_and_save(output.squeeze(),layer_filename)
